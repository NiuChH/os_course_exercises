...
2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？
   1. 各个存储介质都处于满负荷状态而又不至于造成瓶颈
3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？
   1. 修改代码使得内存能及时释放。如果仍不够，则需要将部分数据存储在硬盘。

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？
   1. 将程序划分为功能相互独立的模块，将不会同时执行的模块共享同一块区域
2. 什么是交换技术？覆盖与交换有什么不同？
   1. 将暂时不用的程序放到外存中
   2. 其操作的单位是整个进程的地址空间、不需要模块间的逻辑覆盖结构
3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？
   1. 有需要，如在物理内存不足时，交换技术仍可以使用
4. 如何分析内核模块间的依赖关系？
   1. 使用depmod命令
5. 如何获取内核模块间的函数调用列表？
   1. 静态代码分析和动态函数调用跟踪
...
1. 什么是时间局部性、空间局部性和分支局部性？
   1. 时间局部性：一条指令的一次执行和下次执行，一个数据的一次访问和下次访问都集中在一个较短时间内。
   2. 空间局部性：当前指令和附近的几条指令，当前访问的数据和附近的几条指令访问的数据都集中在一个较小的范围内
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？
   1. 虚拟存储值把当前指令执行所需的部分地址空间放在物理内存中，其余方向外存中
   2. 可以使得应用程序占用更大的虚拟内存空间
2. 虚拟存储需要什么样的支持技术？
   1. 硬件支持地址转换，OS支持置换算法
...
### 8.5 虚拟页式存储
 ...
 1. 什么是虚拟页式存储？缺页中断处理的功能是什么？
     1. 一开始不将程序的所有页面加载到内存，而是根据进程需要动态加载。
     1. 可以辅助加载未在内存中的页
 1. 为了支持虚拟页式存储的实现，页表项有什么修改？
     1. 需要加入驻留位、修改位、访问位、保护位
 1. 页式存储和虚拟页式存储的区别是什么？
